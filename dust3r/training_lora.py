...
def train(args):
    ...
    #
    #
    # name_include=[ "enc_blocks.*.attn.qkv",
    ...
    lora_config = LoraConfig(
        ...
        # lora_alpha=args.lora_alpha,  # Scaling factor (e.g., 16)
        init_lora_weights="pissa",
        r=16,  # Rank of LoRA updates (e.g., 8)
        lora_alpha=32,  # Scaling factor (e.g., 16)
        lora_dropout=0.1,  # Dropout for regularization
        target_modules=target_modules,
        modules_to_save=["patch_embed.proj", "decoder_embed"],  # Train patch embedding and decoder embedding directly
...
def save_final_model(args, epoch, model_without_ddp, best_so_far):
    output_dir = Path(args.output_dir)
