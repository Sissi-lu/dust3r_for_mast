...
def get_patch_embed(patch_embed_cls, img_size, patch_size, enc_embed_dim):
    assert patch_embed_cls in ['PatchEmbedDust3R', 'ManyAR_PatchEmbed', 'ManyAR_PatchEmbed_with_Illumination']
    if patch_embed_cls != 'ManyAR_PatchEmbed_with_Illumination':
        patch_embed = eval(patch_embed_cls)(img_size, patch_size, 3, enc_embed_dim)
    else:
        patch_embed = eval(patch_embed_cls)(img_size, patch_size, 4, enc_embed_dim)
    return patch_embed
...
class ManyAR_PatchEmbed_with_Illumination (PatchEmbed):
    def __init__(self, img_size=224, patch_size=16, in_chans=4, embed_dim=768, norm_layer=None, flatten=True):
        self.embed_dim = embed_dim
        super().__init__(img_size, patch_size, in_chans, embed_dim, norm_layer, flatten)

    def forward(self, img, true_shape, illumination):
        B, C, H, W = img.shape
        assert W >= H, f'img should be in landscape mode, but got {W=} {H=}'
        assert H % self.patch_size[0] == 0, f"Input image height ({H}) is not a multiple of patch size ({self.patch_size[0]})."
        assert W % self.patch_size[1] == 0, f"Input image width ({W}) is not a multiple of patch size ({self.patch_size[1]})."
        ...
        img = torch.concat([img, illumination], dim=1)
