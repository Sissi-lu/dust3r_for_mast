...
from dust3r.model_with_illumination import AsymmetricCroCo3DStereoIllumination, inf  # noqa: F401, needed when loading the model
...
def get_args_parser():
    ...
    parser.add_argument('--model', default="AsymmetricCroCo3DStereoIllumination(patch_embed_cls='ManyAR_PatchEmbed_with_Illumination')",
                        type=str, help="string containing the model to build")
...
def train(args):
    ...
    #----------------load model without patch_embed-----------------#
    if args.pretrained and not args.resume:
        ...
        # Filter out the embedding layer parameters
        pretrained_state_dict = ckpt['model']
        filtered_state_dict = {}

        # Identify embedding layer keys (adjust 'embed' to match your model's naming convention)

        embed_keys = [key for key in pretrained_state_dict.keys() if 'patch_embed' in key.lower()]

        # Copy only non-embedding layer parameters
        for key, value in pretrained_state_dict.items():
            if key not in embed_keys and key in model_state_dict:
                filtered_state_dict[key] = value

        # Load the filtered state dictionary
        model.load_state_dict(filtered_state_dict, strict=False)
        print("Loaded pretrained weights, excluding embedding layer.")
