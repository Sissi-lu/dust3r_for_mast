...
datadir = "/data_new/luxiaoxi/dataset/medical_slam/C3VD_undistorted/desc_t4_a_p2_under_review/depths"
imgs_name = sorted(glob.glob(os.path.join(datadir, "*.tiff")))

for idx, img_name in enumerate(imgs_name):
    num = int(img_name.split('/')[-1].split('.')[0])
    old_name = img_name.split('/')[-1]
    # new_name = "%04d_color.png"%num
    new_name = "%04d_depth.tiff"%num
    new_img_name = img_name.replace(old_name, new_name)
    os.rename(img_name, new_img_name)